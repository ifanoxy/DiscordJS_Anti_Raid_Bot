const { GuildChannel, Client, AuditLogEvent, EmbedBuilder } = require("discord.js");
            ...
            if(await joueur.get(`AntiRaid_ChnDelete_${executor.id}`) < dblimit - 1){
                ...
                switch(sanc.toLocaleLowerCase()){
                    case "ban": {
                        ...
                        .catch()
                        await joueur.delete(`AntiRaid_ChnDelete_${executor.id}`);
                        const fetchLog = await GuildChannel.guild.fetchAuditLogs({
                            limit: config.limits.ChannelDelete,
                            type: AuditLogEvent.ChannelDelete
                        })
                        fetchLog.entries.map(channel => {
                            GuildChannel.guild.channels.create({
                                name: channel.target.name,
                                parent: channel.target.parentId, 
                                topic: channel.target.topic,
                                type: channel.target.type,
                                permissionOverwrites: channel.target.permissionOverwrites,
                                nsfw: channel.target.nsfw,
                                position: channel.target.position
                            }).then(chn => chn.send({embeds:[new EmbedBuilder().setTitle(lang.channelDelete.embed.title).setDescription(lang.channelDelete.embed.description).setColor('Blurple').setTimestamp()]}))
                        })
                    }break;
                    case "kick": {
                        member.kick()
                        .catch(async () => {
                            owner.send(lang.channelDelete.KickCatch2 +`${executor.tag} (${executor.id})`+ lang.channelDelete.KickCatch2).catch()
                        })
